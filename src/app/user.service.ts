...
export class UserService {
  ...
  addUsers(firstname: string, lastname: string, username: string, email: string, phonenumber: number, address: string, zipcode: number, password: string) {
  ...
  updateUser(userId: string, firstname: string, lastname: string,
    username: string, email: string, phonenumber: number, address: string, zipcode: number, password: string) {
    //request path http://localhost:8000/users/5xbd456xx
    //information will be send as HTTP body parameters
    this.http.put("http://localhost:8000/users/"+ userId, {
      firstname, lastname, username, email, phonenumber, address, zipcode, password
    })
      .subscribe(() => {
